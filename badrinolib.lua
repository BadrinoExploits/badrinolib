if game.CoreGui:FindFirstChild("BadrinoLib") then
    game.CoreGui.BadrinoLib:Destroy()
end
game:GetService("UserInputService").InputBegan:connect(
    function(key, gpe)
        if key.KeyCode == Enum.KeyCode.RightControl then
            pcall(
                function()
                    for i, v in pairs(game.CoreGui.BadrinoLib:GetChildren()) do
...
local BadrinooLib = {RainbowColorValue = 0, HueSelectionPosition = 0}
...
coroutine.wrap(
	function()
		while wait() do
			BadrinooLib.RainbowColorValue = BadrinooLib.RainbowColorValue + 1 / 255
			BadrinooLib.HueSelectionPosition = BadrinooLib.HueSelectionPosition + 1

			if BadrinooLib.RainbowColorValue >= 1 then
				BadrinooLib.RainbowColorValue = 0
			end

			if BadrinooLib.HueSelectionPosition == 93 then
				BadrinooLib.HueSelectionPosition = 0
			end
		end
	end
)()
...
local BadrinoLib = Instance.new("ScreenGui")
BadrinoLib.Name = "BadrinoLib"
BadrinoLib.Parent = game.CoreGui
BadrinoLib.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

function BadrinooLib:Window()
				...
				function()
					...
					if RainbowColorPicker then
						...
						while RainbowColorPicker do
							CurrentColor.BackgroundColor3 = Color3.fromHSV(BadrinooLib.RainbowColorValue, 1, 1)
							Color.BackgroundColor3 = Color3.fromHSV(BadrinooLib.RainbowColorValue, 1, 1)

							ColorSelection.Position = UDim2.new(1, 0, 0, 0)
							HueSelection.Position = UDim2.new(0.48, 0, 0, BadrinooLib.HueSelectionPosition)
